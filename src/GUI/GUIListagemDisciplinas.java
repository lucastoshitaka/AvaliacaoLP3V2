...
public class GUIListagemDisciplinas extends JDialog {
JPanel painelTa = new JPanel();
JScrollPane scroll = new JScrollPane();
SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
DecimalFormat decimalFormat = new DecimalFormat("###,###,##0.00");
public GUIListagemDisciplinas(List<Disciplinas> texto) {
...
String[] colunas = new String[]{"IdDisciplinas", "NomeDisciplinas", };
String[][] dados = new String[0][3];
DefaultTableModel model = new DefaultTableModel(dados, colunas);
JTable tabela = new JTable(model);
scroll.setViewportView(tabela);
for (int i = 0; i < texto.size(); i++) {
String[] linha = new String[]{String.valueOf(texto.get(i).getIdDisciplinas()), String.valueOf(texto.get(i).getNomeDisciplinas()), };
model.addRow(linha);
}
painelTa.add(scroll);
